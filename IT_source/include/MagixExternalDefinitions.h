...
struct Critter
{
	...
	Critter()
	{
		...
		dropList.clear();
		skillDrop = pair<String,unsigned char>("",0);
		maxSpeed = 100;
		decisionMin = 1;
		decisionDeviation = 9;
		scale = 1;
...
public:
	...
	bool isRestricted(const short &headID, const short &maneID)
	{
		...
		ifstream inFile;
		inFile.open("MeshRestrictions.cfg", ifstream::in);
		inFile.seekg(0,ios::end);
		tSize = inFile.tellg();
		inFile.seekg(0,ios::beg);
		tBuffer = new char[tSize];
	...
	void loadSettings(bool &isTablet, Real &mouseSensitivity, Real &musicVol, Real &guiVol, Real &soundVol, bool &isShadowsOn, bool &isBloom, String &username, bool &generalCharname, bool &localUsername, bool &channelBlink, bool &runMode, bool &doubleJumpOn, bool &autoLipSync, bool &useWindowsCursor)
	{
		...
		ifstream inFile;
		inFile.open("Settings.cfg", ifstream::in);
		if(inFile.good())
		{
			inFile.seekg(0,ios::end);
			tSize = inFile.tellg();
		...
		ifstream inFile2;
		inFile2.open("useWindowsCursor.dat", ifstream::in);
	...
	void saveSettings(bool isTablet, const Real &mouseSensitivity, const Real &musicVol, const Real &guiVol, const Real &soundVol, const bool &isShadowsOn, bool isBloom, const String &username="", bool generalCharname=false, bool localUsername=false, bool channelBlink=true, bool runMode=true, bool doubleJumpOn=true, bool autoLipSync=false, bool useWindowsCursor=false)
	{
		...
		ofstream outFile;
		outFile.open("Settings.cfg", ofstream::out);
		...
		{
			ofstream outFile2;
			outFile2.open("useWindowsCursor.dat", ofstream::out);
			tBuffer = "1";
	...
	void savePassword(bool flag, const String &password="")
	{
		...
		{
			ofstream outFile;
			outFile.open("Settings2.dat",ios_base::binary);
	...
	const String loadPassword()
	{
		String tPassword = "";
		ifstream inFile;
		inFile.open("Settings2.dat",ios_base::binary);
		if(inFile.good())
		{
			char tBuffer[16] = "";
	...
	void saveBanFile(bool flag, const unsigned short &numDays=0)
	{
		if(flag)
		{
			...
			ofstream outFile;
			outFile.open("Settings3.dat",ios_base::binary);
	...
	const bool loadBanFile(unsigned short &numDays)
	{
		...
		String tDate = "";
		ifstream inFile;
		inFile.open("Settings3.dat",ios_base::binary);
		if(inFile.good())
		{
			char tBuffer[16] = "";
			inFile.getline(tBuffer,16);
	...
	bool XOR7FileGen(const String &infile, const String &outfile, bool decrypt, bool checksum=false)
	{
		...
		ofstream outFile(outfile.c_str(),(decrypt?ofstream::out:ios_base::binary));
	...
	/*bool loadWorld(const String &name, String &terrain, Real &x, Real &z, Vector3 &spawnSquare, String &grassMat, String &grassMap, String &grassColourMap, String &tree1, String &tree2, String &tree3, unsigned short &treeCount, String &bush1, String &bush2, String &bush3)
	{
		...
		ifstream inFile;
		inFile.open("Worlds.cfg", ifstream::in);
		inFile.seekg(0,ios::end);
		tSize = inFile.tellg();
		inFile.seekg(0,ios::beg);
		tBuffer = new char[tSize];
		...
		inFile.open("CustomWorlds.cfg", ifstream::in);
		inFile.seekg(0,ios::end);
		tSize = inFile.tellg();
		inFile.seekg(0,ios::beg);
		tBuffer = new char[tSize];
	...
	void loadCampaignList(vector<String> &list, bool customCampaigns=false)
	{
		...
		ifstream inFile;
		inFile.open(customCampaigns?"CustomCampaigns.cfg":"Campaigns.cfg", ifstream::in);
		if(!inFile.good())
		{
	...
	{
		ifstream inFile;
		inFile.open( (isSavePoint?"savepoint.sav":"homepoint.sav") , ifstream::in);
	...
	{
		ofstream outFile;
		outFile.open( (isSavePoint?"savepoint.sav":"homepoint.sav") , ofstream::out);
	...
	void saveBio(const String &name, const String &bio)
	{
		...
		outFile.open(tFilename.c_str(), ofstream::out);
	...
	const String loadBio(const String &name="")
	{
		...
		inFile.open( tFilename.c_str(), ifstream::in);
	...
	const String loadUpdateCaption()
	{
		...
		ifstream inFile;
		inFile.open( "update.txt", ifstream::in);
		if(!inFile.good())return "";
		inFile.seekg(0,ios::end);
		tSize = inFile.tellg();
		inFile.seekg(0,ios::beg);
	...
	const pair<Vector3,bool> getItemParticleOffset(const String &meshName)
	{
		for(int i=0;i<maxItems;i++)
			if(itemMesh[i]==meshName && (int)itemParticleOffset.size()>i)
			{
				if((int)itemParticleOnNode.size()>i)return pair<Vector3,bool>(StringConverter::parseVector3(itemParticleOffset[i]),itemParticleOnNode[i]);
				else return pair<Vector3,bool>(StringConverter::parseVector3(itemParticleOffset[i]),false);
			}
		return pair<Vector3,bool>(Vector3::ZERO,false);
	}
	...
	void loadWeatherCycle(const String &type, vector<const WeatherEvent> &list, bool isCustom=false)
	{
		...
		ifstream inFile;
		inFile.open(isCustom?"../../media/weather/CustomWeatherCycles.cfg":"../../media/weather/WeatherCycles.cfg", ifstream::in);
		if(!inFile.good())return;
		inFile.seekg(0,ios::end);
		tSize = inFile.tellg();
		inFile.seekg(0,ios::beg);
	...
	void loadWeather(const String &type, String &particle, Vector3 &offset, String &sound, bool isCustom=false)
	{
		...
		ifstream inFile;
		inFile.open(isCustom?"../../media/weather/CustomWeathers.cfg":"../../media/weather/Weathers.cfg", ifstream::in);
		if(!inFile.good())return;
		inFile.seekg(0,ios::end);
		tSize = inFile.tellg();
		inFile.seekg(0,ios::beg);
	...
	const pair<String,unsigned char> getCritterSkillDrop(const String &type)
	{
		...
		return pair<String,unsigned char>("",0);
	}
	const pair<String,unsigned char> getRandomSkillDrop()
	{
		...
		switch(tID)
		{
		case 0: return pair<String,unsigned char>("Cure",10);
		case 1: return pair<String,unsigned char>("Lunge",10);
		case 2: return pair<String,unsigned char>("Sweep",10);
		case 3: return pair<String,unsigned char>("Stomp",10);
		case 4: return pair<String,unsigned char>("Backflip",10);
		case 5: return pair<String,unsigned char>("Tame",3);
		case 6: return pair<String,unsigned char>("Roar",6);
		case 7: return pair<String,unsigned char>("Zoom",10);
		}
		return pair<String,unsigned char>("",0);
	}
	...
	const pair<CritterAttack,String> getCritterAttackAndSound(const String &type, const unsigned char &iID)
	{
		for(int i=0;i<(int)critterList.size();i++)
		{
			if(critterList[i].type==type)
			{
				if(iID>=0 && iID<(int)critterList[i].attackList.size())return pair<CritterAttack,String>(critterList[i].attackList[iID],critterList[i].sound);
				else return pair<CritterAttack,String>(CritterAttack(0,0,Vector3::ZERO),critterList[i].sound);
			}
		}
