		...
		if(spawnTimer<=0 && mCritterManager->getNumCritters()<mWorld->getCritterSpawnLimit())
		{
			...
			if(tList.size()>0)
			{
				...
				if(Math::UnitRandom()<(tCritter.spawnRate))
				{
					...
					{
						const pair<Vector3,Vector3> tRoamArea = mWorld->getCritterRoamArea(tCritter.roamAreaID);
			...
			else
			{
				...
				if(tC->getHasAttack() && tTargetUnit && Math::UnitRandom()<0.7)
				{
					...
					else
					{
						...
						if(tC->getRoamArea())
						{
							const pair<Vector3,Vector3> tRoamArea = mWorld->getCritterRoamArea(tC->getRoamAreaID());
