...
public:
	...
	bool equipItem(MagixUnit *unit, const String &meshName, const short &slot)
	{
		...
		if(mDef->getItemParticle(meshName)!="")
		{
			...
			if(tEnt)
			{
				const pair<Vector3,bool> tOffsetAndOnNode = mDef->getItemParticleOffset(meshName);
	...
	{
		partyMembers.push_back(pair<OwnerToken,String>(token,name));
		partyChanged = true;
	}
	...
	const pair<OwnerToken,String> getPartyMember(const OwnerToken &token)
	{
		for(int i=0;i<(int)partyMembers.size();i++)
