...
struct Portal
{
	...
	bool collides(const AxisAlignedBox &target)
	{
		...
		if(mEnt->getWorldBoundingSphere().intersects(target))return true;
		return false;
	}
	void disable()
	{
...
public:
	...
	void createPortal(const Vector3 &center, const Real &range, String destName)
	{
		...
		tPortal.mNode->scale(range,range,range);
	...
	void getPortalMap(Ogre::vector<pair<Vector2,String>>::type &map)
	{
		...
		while(it!=portal.end())
		{
			...
			if(tPortal->mNode)
			{
				const pair<Vector2,String> tData = pair<Vector2,String>(Vector2(tPortal->mNode->getPosition().x,tPortal->mNode->getPosition().z),tPortal->dest);
				map.push_back(tData);
			}
			it++;
		}
	...
	void getGateMap(Ogre::vector<pair<Vector2,String>>::type &map)
	{
		...
		while(it!=gate.end())
		{
			...
			if(tGate->mNode)
			{
				const pair<Vector2,String> tData = pair<Vector2,String>(Vector2(tGate->mNode->getPosition().x,tGate->mNode->getPosition().z),tGate->dest);
				map.push_back(tData);
			}
			it++;
		}
