...
public:
		...
		if(tBuffer.length()>0)
		{
			...
			for(int i=0;i<int(tPart.size());i++)
			{
				...
				if(tLine.size()>0)
				{
					...
					{
						for(int j=1;j<(int)tLine.size();j++)tTreesFilename.push_back(pair<String,bool>(tLine[j],false));
					}
					else if(StringUtil::startsWith(tLine[0],"Bushes",false) && tLine.size()>=2)
					{
						for(int j=1;j<(int)tLine.size();j++)tBushesFilename.push_back(pair<String,bool>(tLine[j],false));
					}
					else if(StringUtil::startsWith(tLine[0],"FloatingBushes",false) && tLine.size()>=2)
					{
						for(int j=1;j<(int)tLine.size();j++)tFloatingBushesFilename.push_back(pair<String,bool>(tLine[j],false));
					}
					else if(StringUtil::startsWith(tLine[0],"NewTrees",false) && tLine.size()>=2)
					{
						for(int j=1;j<(int)tLine.size();j++)tTreesFilename.push_back(pair<String,bool>(tLine[j],true));
					}
					else if(StringUtil::startsWith(tLine[0],"NewBushes",false) && tLine.size()>=2)
					{
						for(int j=1;j<(int)tLine.size();j++)tBushesFilename.push_back(pair<String,bool>(tLine[j],true));
					}
					else if(StringUtil::startsWith(tLine[0],"NewFloatingBushes",false) && tLine.size()>=2)
					{
						for(int j=1;j<(int)tLine.size();j++)tFloatingBushesFilename.push_back(pair<String,bool>(tLine[j],true));
					}
	...
	const pair<Vector3,Vector3> getCritterRoamArea(const unsigned short &iID)
	{
		if(iID<0 || iID>=critterRoamAreaList.size())return pair<Vector3,Vector3>(Vector3::ZERO,Vector3::ZERO);
