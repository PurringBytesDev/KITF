...
public:
	MagixNetworkManager()
	{
		...
		lastMovePosition = pair<Vector3,Real>(Vector3::ZERO,0);
		lastFaceDirection = Degree(0);
		lastLookQuaternion = pair<bool,Quaternion>(false,Quaternion::IDENTITY);
		lastFlags = 0;
		lastPrimaryFlags = 0;
	...
	void processPosition(Packet *p)
	{
		...
		if(!tUnit)
		{
			playerPositionQueue.push_back(pair<OwnerToken,PositionInfo>(tToken,PositionInfo(tPosition,tHasGroundHeight,tGroundHeight)));
			return;
		}
	...
	void processInfo(Packet *p)
	{
		...
		else if(type==INFO_PARTYJOINED)
		{
			...
			const pair<OwnerToken,String> tCheck = mUnitManager->getPartyMember(tToken);
		...
		{
			const pair<OwnerToken,String> tMember = mUnitManager->getPartyMember(tToken);
	...
	/*void sendSkillInitialize()
	{
		...
		tBitStream.Write(true);
		const vector<Skill> tSkill = mUnitManager->getPlayer()->getSkills();
	...
	void processServerList(Packet *p)
	{
		...
		while(tHasMore)
		{
			...
			if(!tFull)serverAdd.push_back(pair<unsigned char,SystemAddress>(tID,tAdd));
			tReceiveBit.Read(tHasMore);
		}
	}
	void processServerConnected(Packet *p)
	{
		...
		if(tIsConnect)
		{
			serverAdd.push_back(pair<unsigned char,SystemAddress>(tID,tAdd));
		}
		else
		{
			for(vector<pair<unsigned char,SystemAddress>>::iterator it=serverAdd.begin();it!=serverAdd.end();it++)
