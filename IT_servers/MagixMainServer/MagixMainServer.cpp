...
public:
				...
				case ID_TOKENCONNECTED:
					{
						...
						if(tToken==0)
						{
							...
							char tBuffer[128] = "";
							strcpy_s(tBuffer,"Failed to assign token for ");
							outFile.write(tBuffer,strlen(tBuffer));
							strcpy_s(tBuffer,tAdd.ToString());
							outFile.write(tBuffer,strlen(tBuffer));
							strcpy_s(tBuffer,"\n");
							outFile.write(tBuffer,strlen(tBuffer));
							outFile.close();
				...
				case ID_LOGON:
					{
						...
						if(tLogonSuccess)
						{
							...
							if(outFile.good())
							{
								...
								time(&rawtime);
								char tBuffer[128] = "";
								strcpy_s(tBuffer,p->systemAddress.ToString());
								outFile.write(tBuffer,strlen(tBuffer));
								strcpy_s(tBuffer,",");
								outFile.write(tBuffer,strlen(tBuffer));
								strcpy_s(tBuffer,tUsername);
								outFile.write(tBuffer,strlen(tBuffer));
								strcpy_s(tBuffer,",");
								outFile.write(tBuffer,strlen(tBuffer));
								strcpy_s(tBuffer,"");
								ctime_s(tBuffer,128,&rawtime);
								outFile.write(tBuffer,strlen(tBuffer));
							}
							outFile.close();
							...
							for(vector<const pair<string,SystemAddress>>::iterator it=loginSession.begin();it!=loginSession.end();it++)
							{
								const pair<string,SystemAddress> tSession = *it;
						...
						if(strlen(tUsername)>0 && strlen(tPassword)>0)
						{
							...
							if(!inFile.good())
							{
								...
								if(!inFile.good())
								{
									...
									if(!inFile.good())tCreateSuccess = true;
								}
							}
							inFile.close();
							if(tCreateSuccess)
				...
				case ID_LOADCHAR:
					{
						...
						if(tLoadSuccess)
						{
							...
							for(int j=0;j<MAX_EQUIP;j++)strcpy_s(tItem[j],"");
							char tHP[16] = "";
							char tSkills[512] = "";
							char tPet[32] = "";
				...
				case ID_DELETECHAR:
					{
						...
						if(tSuccess)
						{
							...
							_unlink(getFilename(tFilename.c_str(),".character").c_str());
							//Delete item file
							_unlink(getFilename(tFilename.c_str(),".item").c_str());
						}
				...
				case ID_VIOLATION:
					{
						...
						if(tToken>0 && tToken<=MAX_CLIENTS)
						{
							strcpy_s(tBuffer," -> ");
							outFile.write(tBuffer,strlen(tBuffer));
							strcpy_s(tBuffer,playerToken[tToken-1].name.c_str());
							outFile.write(tBuffer,strlen(tBuffer));
							strcpy_s(tBuffer," [");
							outFile.write(tBuffer,strlen(tBuffer));
							strcpy_s(tBuffer,"");
							_itoa_s((int)playerToken[tToken-1].charID,tBuffer,10);
							outFile.write(tBuffer,strlen(tBuffer));
							strcpy_s(tBuffer,"]");
							outFile.write(tBuffer,strlen(tBuffer));
						}
	...
	const string getFilename(const char *name, const char *fileExtension, bool replaceSpaces=true, bool replaceUnderscores=false)
	{
		...
		{
			if(name[0]>='a')firstChar[0] = name[0]-'a'+'A';
			firstChar[0] = name[0];
		}
	...
	void savePlayerData(const OwnerToken &token)
	{
		...
		char tHP[6] = "";
		char tMaxHP[6] = "";
		sprintf_s(tHP,"%i",playerToken[token-1].hp.first);
		sprintf_s(tMaxHP,"%i",(playerToken[token-1].hp.second==0?500:playerToken[token-1].hp.second));
		string tHPStr = tHP;
		tHPStr += ";";
		tHPStr += tMaxHP;
		...
		for(int j=0;j<(int)playerToken[token-1].skill.size();j++)
		{
			...
			tSkill += ";";
			char tStock[4] = "";
			sprintf_s(tStock,"%i",playerToken[token-1].skill[j].second);
			tSkill += tStock;
			tSkill += "|";
		}
		if(tSkill.length()>=512)
		{
			...
			ofstream outFile("./exceptions.log",ios_base::app);
			char tBuffer[128] = "";
			_itoa_s((int)token,tBuffer,10);
			outFile.write(tBuffer,strlen(tBuffer));
			strcpy_s(tBuffer,playerToken[token-1].name.c_str());
			outFile.write(tBuffer,strlen(tBuffer));
			strcpy_s(tBuffer," skill length longer than 512\n");
			outFile.write(tBuffer,strlen(tBuffer));
			outFile.close();
		}
	...
	void saveBanlist()
	{
		...
		for(int i=0;i<(int)banlist.size();i++)
		{
			char tYear[6] = "";
			char tDay[6] = "";
			_itoa_s(banlist[i].year,tYear,6,10);
			_itoa_s(banlist[i].yDay,tDay,6,10);
	...
	void savePingRange()
	{
		...
		_itoa_s(lowPing,tLowPing,16,10);
		_itoa_s(highPing,tHighPing,16,10);
