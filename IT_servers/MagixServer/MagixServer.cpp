...
public:
	ServerManager()
	{
		...
		{
			broadcastAdd = "";
			mainServerInitAdd = MAIN_SERVER_IP;
		}
	}
	~ServerManager()
	{
	...
	bool initialize(const unsigned int &sleepTime=0)
	{
		...
		SocketDescriptor socketDescriptor(SERVER_PORT+connectAttempts,0);
				...
				case ID_FORCELOGOUT:
					{
						...
						{
							for(vector<const string>::iterator it=bootlist.begin();it!=bootlist.end();it++)
							{
				...
				case ID_SERVERLIST:
					{
						...
						vector<const pair<unsigned char,SystemAddress>> tServerList;
						bool tHasMore = false;
